...
function ProImage() {
  ...
  return (
    <div className="pt-28 pl-28">
      ...
      <div className="pt-12 flex flex-row justify-between w-[860px] gap-4">
      ...
      <div className="flex w-[860px] mt-12 border border-[#DCDCEB] rounded-xl divide-x divide-[#DCDCEB] bg-white dark:bg-gray-900 shadow-sm">
        {details.map((item, index) => (
          <div
            key={index}
            className="flex items-center justify-center gap-2 flex-1 py-4 text-[#585981] dark:text-gray-200"
          >
            {item.icon}
            <span className="text-[17px] font-[600]">{item.value}</span>
          </div>
        ))}
      </div>
    </div>
